...
export default function AdminTournamentPage() {
    ...
    const overallRanking = useMemo(() => {
        ...
        const base = activeParticipants.map((p) => ({
            ...
            savedPlace: null as number | null,
        ...
        // ✅ Map rapid: player_id -> registration row (folosim snapshot-urile din registrations)
        // IMPORTANT: aici `rows` este lista de registrations pentru acest turneu
        const regMap = new Map(rows.map((r) => [r.player_id, r]));

        const idToRow = new Map(base.map((x) => [x.id, x]));

        // Dacă locurile au fost salvate în DB, afișăm strict după registrations.final_place
        for (const row of base) {
            const reg = regMap.get(row.id);
            if (reg) {
                row.savedPlace = reg.final_place ?? null;
                // dacă există mp_turneu salvat, îl folosim pentru afișare (nu recalculăm)
                row.mpTournament = reg.mp_turneu ?? null;
            }
        }

        const useSavedPlaces = base.some((x) => x.savedPlace != null);
        ...
        return sorted;
    }, [activeParticipants, matchesKO, podium, groupsUpper, groupsLower, rows]);
